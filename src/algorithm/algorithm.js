...
export class Algorithm {
    ...
    /***********************************************/
    static get is() { return 'rhi-core-isncsci-algorithm'; }
    /***********************************************/
    ...
    */
    static updateTotalsWithLevelAt(isncsciExam, totals, level, nextNonKeyMuscleShouldBeRightMotor, nextNonKeyMuscleShouldBeLeftMotor) {
        ...
        /* -- RECURSIVE CALL --------------- */
        if (level.next != null)
            Algorithm.updateTotalsWithLevelAt(isncsciExam, totals, level.next, nextNonKeyMuscleShouldBeRightMotor && totals.rightMotorHasOnlySoftValues, nextNonKeyMuscleShouldBeLeftMotor && totals.leftMotorHasOnlySoftValues);
    ...
    */
    static couldNotHaveMotorFunctionMoreThan3LevelsBelowMotorLevel(isncsciExam, totals) {
    ...
    */
    static couldBeAsiaCorD(isncsciExam, totals) {
        let couldBeAsiaC = false;
        let couldBeAsiaD = false;
        let rightMotor = null;
        let leftMotor = null;
        ...
        const couldHaveAnalContraction = isncsciExam.analContraction == BinaryObservation.yes
        ...
        while (index < levelsLength && (!couldBeAsiaC || !couldBeAsiaD)) {
            ...
            let levelsWithMuscleGradeGreaterThanTwo = 0;
            let levelsWithMuscleGradeLessThanThree = 0;
            let eligibleLevelCount = 0;
            let currentLevel = currentNli.next;
    ...
    */
    static getTotalsFor(isncsciExam) {
        if (!isncsciExam)
            throw new Error(`${Algorithm.is} : getTotalsFor : 
                            Missing the expected argument isncsciExam:IsncsciExam`);
        const totals = new IsncsciTotals();
        const c2 = isncsciExam.getLevelWithName('C2');
        Algorithm.updateTotalsWithLevelAt(isncsciExam, totals, c2, false, false);
        const s4_5 = isncsciExam.getLevelWithName('S4_5');
        const c1 = c2.previous;
        ...
        if (totals.mostRostralNeurologicalLevelOfInjury.name !== 'S4_5'
            ...
            const cdCheck = Algorithm.couldBeAsiaCorD(isncsciExam, totals);
...
Algorithm.ntRegex = /\bNT\b/i;
Algorithm.notDeterminable = 'UTD';
Algorithm.notApplicable = 'NA';
Algorithm.intact = 'INT';
