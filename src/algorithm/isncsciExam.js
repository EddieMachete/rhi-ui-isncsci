...
export class IsncsciExam {
    constructor() {
        ...
        /* ***** Public properties ********** */
        /***********************************************/
        this.analContraction = null;
        this.analSensation = null;
        this.rightLowestNonKeyMuscleWithMotorFunction = null;
        this.leftLowestNonKeyMuscleWithMotorFunction = null;
        this.levels = IsncsciExam.getIsncsciExamLevels();
    }
    /***********************************************/
    /* ***** Private, static properties ********** */
    /***********************************************/
    static get is() { return 'rhi-core-isncsci-exam'; }
    /***********************************************/
    ...
    * @return {NeuroLevel[]}
    */
    static getIsncsciExamLevels() {
        let levels = [];
        let c1 = new NeuroLevel();
        c1.name = 'C1';
        c1.previous = null;
        c1.setValues(0, false, false, '2', 2, false, '2', 2, false, '2', 2, false, '2', 2, false, '5', 5, false, '5', 5, false);
        levels.push(c1);
    ...
    */
    static getDermatomeValue(text, normalValue) {
    ...
    */
    static updateLevel(level, rightTouch, leftTouch, rightPrick, leftPrick, rightMotor, leftMotor) {
        // Right Touch
    ...
    */
    getLevelWithName(levelName) {
        const levelNameUpper = levelName.toUpperCase();
        return this.levels.find((level) => level.name === levelNameUpper);
    }
    /**
    ...
    */
    setRightLowestNonKeyMuscleWithMotorFunction(levelName) {
        const key = levelName.toUpperCase();
        const level = this.getLevelWithName(key);
        if (!level)
    ...
    */
    setLeftLowestNonKeyMuscleWithMotorFunction(levelName) {
        const key = levelName.toUpperCase();
        const level = this.getLevelWithName(key);
        if (!level)
    ...
    */
    updateLevelByName(levelName, rightTouch, leftTouch, rightPrick, leftPrick, rightMotor, leftMotor) {
...
IsncsciExam.ntRegex = /\bNT\b/i;
IsncsciExam.nonSciImpairmentRegex = /.+[!]/;
IsncsciExam.nonSciImpairmentFlagsRegex = /[\\*!]/;
IsncsciExam.normalMotorValue = 5;
IsncsciExam.normalSensoryValue = 2;
